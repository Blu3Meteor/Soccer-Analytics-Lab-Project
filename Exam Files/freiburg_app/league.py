...
def render_league_table_page(squads: dict[int, dict[str, Any]]) -> None:
    ...
    with st.expander("Regression coefficients", expanded=False):
        st.caption(
            ...
            "GD is actual goal difference from the results table; xGD is the model's expected goal difference, so they are related but not the same."
        )
    ...
    for row in rows:
        ...
        html_rows.append(
            ...
            f'<td class="league-table-model">{model_row["expected_goals_for"]:.2f}</td>'
            f'<td class="league-table-model">{model_row["expected_goals_against"]:.2f}</td>'
    ...
    st.markdown(
        ...
        '<th>Model Rank</th><th>Expected Pts</th><th>Exp GF</th><th>Exp GA</th><th>Expected GD</th><th>Attack</th><th>Concede</th>'
        '</tr></thead>'
    ...
    st.caption(
        ...
        "Expected Pts = predicted league points; Exp GF / Exp GA = modelled goals for and against; Expected GD = Exp GF minus Exp GA, shown separately from actual GD; "
